...
// ADR-0053: "An Album corroborates its Artist." Ported from
// internal/enrich/artist_corroboration_test.go — the stub, the fixtures and the
// assertions are that file's; the pass-level tests at the bottom of it stayed in
// the host, because they are about the pass.
//
...
// "Eagles". artistDetails searched artist:"The Eagles" as an exact phrase, took
// Artists[0], and stored it with no acceptance test of any kind — so the row read
// `matched`, nothing ever flagged it, and the damage surfaced as thirteen unmatched
// tracks on Hell Freezes Over three levels down.
//
...
const (
	...
	// americanEaglesMBID stands for the band that actually recorded the album.
...
// wrong one's.
func theEaglesStub(t *testing.T) (*Provider, *mbArtistStub) {
	t.Helper()
	stub := &mbArtistStub{
		artists: []stubArtist{
	...
	p, _ := newProvider(t, stub.serve, noPacing())
	return p, stub
}

// artistRef builds the ref the host hands the provider for an Artist that carries
// no id of its own.
func artistRef(name string, hints ...pluginapi.AlbumHint) pluginapi.MediaRef {
	return pluginapi.MediaRef{Kind: "artist", Title: name, Artist: name, AlbumHints: hints}
}
...
func TestTheEaglesResolveThroughTheirAlbumNotTheirName(t *testing.T) {
	...
	meta, err := lookup(p, artistRef("The Eagles", pluginapi.AlbumHint{Title: "Hell Freezes Over"}))
	if err != nil {
...
func TestATaggedAlbumCorroboratesItsArtistWithNoSearchAtAll(t *testing.T) {
	...
	meta, err := lookup(p, artistRef("The Eagles",
		pluginapi.AlbumHint{Title: "Hell Freezes Over", ReleaseGroupMBID: hellFreezesOverRGID}))
	if err != nil {
		t.Fatalf("lookup: %v (calls: %v)", err, stub.calls())
	}
...
func TestTheCorroboratingAlbumSearchIsUnnarrowedOnTheWire(t *testing.T) {
	...
	if _, err := lookup(p, artistRef("The Eagles", pluginapi.AlbumHint{Title: "Hell Freezes Over"})); err != nil {
...
// shape a relevance query returns constantly. normalizeMatchTitle refuses it, and
...
func TestAnArtistWithNoIdentifiableAlbumsAsksExactlyWhatItAlwaysDid(t *testing.T) {
	for _, tc := range []struct {
		name  string
		hints []pluginapi.AlbumHint
	}{
		{"no albums at all", nil},
		{"albums with neither an id nor a title", []pluginapi.AlbumHint{{}, {Title: "   "}}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			p, stub := theEaglesStub(t)

			meta, err := lookup(p, artistRef("The Eagles", tc.hints...))
			if err != nil {
				t.Fatalf("lookup: %v", err)
			}
			if meta.ExternalID != britishEaglesMBID {
...
func TestAnArtistWithNoNameAndNoAlbumsAsksNothing(t *testing.T) {
	...
	if _, err := lookup(p, artistRef("  ")); !errors.Is(err, errNoMatch) {
...
func TestCorroborationReplacesTheNameSearchRatherThanJoiningIt(t *testing.T) {
	cases := []struct {
		...
		{"the files name the release-group", pluginapi.AlbumHint{Title: "Hell Freezes Over", ReleaseGroupMBID: hellFreezesOverRGID}, 1, 0, 0},
		{"only the album title is known", pluginapi.AlbumHint{Title: "Hell Freezes Over"}, 1, 1, 0},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			p, stub := theEaglesStub(t)

			if _, err := lookup(p, artistRef("The Eagles", tc.hint)); err != nil {
				t.Fatalf("lookup: %v", err)
			}
...
// and the rest are never asked about.
func TestOnlyOneHintIsEverSpentAndTheTaggedOneWins(t *testing.T) {
	p, stub := theEaglesStub(t)

	meta, err := lookup(p, artistRef("The Eagles",
		pluginapi.AlbumHint{Title: "Their Greatest Hits"},
		pluginapi.AlbumHint{Title: "Hell Freezes Over", ReleaseGroupMBID: hellFreezesOverRGID},
		pluginapi.AlbumHint{Title: "Hotel California"},
	))
	if err != nil {
		t.Fatalf("lookup: %v (calls: %v)", err, stub.calls())
	}
...
func TestAStaleTaggedReleaseGroupFallsThroughToTheNameSearch(t *testing.T) {
	...
	meta, err := lookup(p, artistRef("The Eagles",
		pluginapi.AlbumHint{Title: "Hell Freezes Over", ReleaseGroupMBID: "deadbeef-0000-0000-0000-000000000000"}))
	if err != nil {
		t.Fatalf("lookup: %v (calls: %v)", err, stub.calls())
	}
	if meta.ExternalID != britishEaglesMBID {
...
func TestAMalformedTaggedReleaseGroupIsNeverSent(t *testing.T) {
	...
	if _, err := lookup(p, artistRef("The Eagles",
		pluginapi.AlbumHint{Title: "Hell Freezes Over", ReleaseGroupMBID: "not-a-uuid"})); err != nil {
		t.Fatalf("lookup: %v", err)
	}
...
//
// Under the contract that outage is `unavailable` and NOT a Go error (ADR-0059
// decision 6): the item takes ADR-0048's backoff and the plugin takes no strike.
func TestATransientCorroborationFailureDoesNotFallBackToTheName(t *testing.T) {
	...
	_, err := lookup(p, artistRef("The Eagles",
		pluginapi.AlbumHint{Title: "Hell Freezes Over", ReleaseGroupMBID: hellFreezesOverRGID}))
	if errors.Is(err, errNoMatch) {
		t.Fatalf("err = %v, want the unavailable answer — 'the discography says nothing' and "+
			"'MusicBrainz was busy' are different answers, and only the first may reach the "+
			"name search (calls: %v)", err, stub.calls())
	}
	assertUnavailable(t, err, "a shed corroborating lookup")
	if n := stub.nameSearches(); n != 0 {
