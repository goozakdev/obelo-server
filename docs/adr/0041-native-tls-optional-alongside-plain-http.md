...
- **HTTP/2 arrives for free** on the TLS listener, which suits HLS: many small segment fetches multiplex over one connection instead of queueing against the per-origin limit. The plain-HTTP listener stays HTTP/1.1. **It holds because `http.Server.ServeTLS` negotiates `h2` when `TLSNextProto` is left alone, so it does not come for free on a listener somebody else wrapped.** The tailnet listener of [ADR-0043](./0043-tailnet-remote-access-via-embedded-tsnet.md) originally lost HTTP/2 for exactly that reason — `tsnet.ListenTLS` hands back an already-TLS-wrapped listener whose `NextProtos` is empty. **That was recorded here as an inherent limitation, and it was not**: `tsnet`'s certificate hook is public and stable, so ADR-0043 now builds its own `tls.Config` with `NextProtos` set and the tailnet has HTTP/2 too. Corrected 2026-08-14; see that ADR's consequences for the correction and for the reading habit that produced the wrong claim.
