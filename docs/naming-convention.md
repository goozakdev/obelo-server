...
- **Optional embedded external id** in the folder name: `Dune (2021) {tmdb-438631}` or `{imdb-tt1160419}`. When present it *is* the identity (overrides title+year parsing) and is also the Enrichment key by default. This is the escape hatch for same-title-same-year collisions and for pinning the exact external record. It stays the identity for good — but only decorates the item until someone says otherwise: an Admin's **Fix info** outranks it as the Enrichment key and survives every later scan ([ADR-0045](./adr/0045-enrichment-record-is-its-own-column.md)). Renaming the folder is how you change the *identity*.
...
- **Collision rule** — two files that parse to the same Edition identity and are not parts are flagged ambiguous in the web app, never silently guessed. "Not parts" is the whole test: an Edition is played back-to-back only when *every* one of its files is numbered (by a part suffix, or by the order an Admin gave them in the file matcher). Until the collision is settled the Edition plays its **first** file and nothing else — joining unnumbered files would play `S01E05-E06` and then `S01E06` again, or the same rip twice, as one continuous work. The flag appears in the Admin's Needs-Fixing queue, which names the colliding files; an ambiguous Episode folds into its Show's row, whose fix is the file matcher.
...
A filename is the *default* way to say any of this, not the only one. Where the names on disk can't say it — a two-part episode whose files carry no `part` suffix, a combined file named nothing like `S01E05-E06`, a season the provider numbers differently — an Admin can say the same things by hand in the file matcher, which records a **Placement** ([ADR-0044](./adr/0044-file-anchored-placement-override-replayed-by-the-scanner.md)) and produces exactly the structures described above. Nothing on disk is renamed or moved to make that work, and a filename later corrected at the source takes effect: Placements are stored only where they differ from the parse.
