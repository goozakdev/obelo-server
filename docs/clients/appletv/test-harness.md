...
- The server binds plain HTTP by default; TLS is either a reverse proxy's job or Obelo's own (`OBELO_TLS_MODE`, ADR-0041). Testing the `Secure`-cookie path needs real TLS, or a proxy setting `X-Forwarded-Proto: https` **from an address listed in `OBELO_TRUSTED_PROXIES`** — the header is ignored from anyone else.
