...
| Endpoint | Notes |
| --- | --- |
| `GET /libraries/{id}/needs-review` — [Admin] | → `200` `{ "items": [ { "id", "kind": "movie\|episode\|track\|show", "title", "year"?, "folderPath"?, "reason"?, "enrichmentStatus"?, …fix context } ] }`. `enrichmentStatus` says whether there is a matched record to confirm the filing against (a never-enriched Show reports `pending`, matching a Title's column default, rather than an empty string). Titles and Shows the scanner filed from an uncertain parse. `folderPath` is the anchor a fix-match must use, present only for the kinds a folder override can fix (Movie, Show, Track) — an Episode has none. `reason` ∈ `no-year` \| `episode-numbering` \| `untagged`, read off the rule the scanner applied, so a client can state the problem instead of the word "needs review". |
| `GET /libraries/{id}/unmatched` — [Admin] | → `200` `{ "files": [ { "id", "path", "folderPath"?, "reason"?, "addedAt"? } ] }`. Recognized media from which no identity could be extracted — no Title exists, so these carry no fix context. `folderPath` is the fix-match anchor, derived from the **Library's kind** (movie → the movie folder or the loose file, tv → the Show folder above the Season folder, music → the album folder); a client must not derive it from the path, where the file's own directory is right only for a Movie. |
