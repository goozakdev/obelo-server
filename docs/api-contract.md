...
| `POST /settings/plugins` — [Admin] | **multipart/form-data** with two file parts: `manifest` (the `manifest.json`, byte for byte as the author wrote it) and `module` (the `.wasm`). Two named parts rather than an archive, so the server never unpacks a path it did not choose. → `201` (GET shape, the whole list). The module is compiled in a staging directory **before** anything is installed, so a refusal leaves nothing on disk and nothing in the database. Errors: `422 PLUGIN_INVALID_MANIFEST` / `422 PLUGIN_API_VERSION` (the message **names which side to upgrade**) / `409 PLUGIN_DUPLICATE` (the id is already claimed, by an Installed plugin or a Built-in) / `422 PLUGIN_INVALID_MODULE` (no module, or one that will not compile or instantiate) / `400 BAD_REQUEST` (a missing part, named) / `413` (a part over the cap: 64 MiB for the module, 256 KiB for the manifest). **Takes effect with no restart** — the registry is rebuilt and swapped, and the provider, subtitle and sink Managers reload. |
| `POST /settings/plugins/from-url` — [Admin] | `{ "url" }` — the URL of a plugin's `manifest.json`; its module is fetched from **beside it** (the manifest's `module` name, default `plugin.wasm`, resolved against the manifest's directory). → `201` (GET shape). Both fetches go through `internal/safefetch`, so the bounded redirect chain and the private-address refusal on every hop apply. **Unlike every other outbound fetch in this server, the first hop is address-checked too**: a URL resolving into loopback / RFC1918 / link-local space is refused, because what comes back is code this server executes. Upload the file instead. Errors: `422 PLUGIN_SOURCE_REFUSED` (not an absolute http(s) URL, unresolvable, a private address, a non-200, or a response over the cap), plus every error the upload can give. |
