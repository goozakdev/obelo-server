...
**A linked Show, Artist and Album advertise their artwork URLs exactly like a local one** — a Show's `posterUrl`/`backgroundUrl`/`logoUrl`, an Artist's `artworkUrl`/`backgroundUrl`/`logoUrl`, an Album's `hasArtwork` — for whichever roles the sharer has, and each carries the sharer's `?v={version}` cache-buster ([ADR-0056](./adr/0056-a-linked-library-is-a-read-only-mirror-played-through-a-one-hop-relay.md) §5, .scratch/linked-servers issue 19). GETting one relays the image bytes from the sharer on first request and serves this Server's own cache after, so a client cannot tell a mirrored poster from a local one. A role the sharer lacks is simply not advertised. Movie posters and Season posters are not yet mirrored (the same relay works for them once their export carries the signal).
