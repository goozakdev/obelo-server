...
| `GET /settings/event-sinks` — [Admin] | → `200` `{ "sinks": [ { "slug", "name", "requiresSecret", "enabled", "hasSecret", "url", "events": ["scan.completed"…], "description", "docsURL" } ], "availableEvents": ["scan.completed"…] }`. Event sinks ([ADR-0057](./adr/0057-plugins-implement-a-closed-set-of-extension-points-through-a-wire-shaped-contract.md) decision 6). The signing secret is **never returned** — only `hasSecret`. `availableEvents` is the event types **this server can actually derive**, which is narrower than the contract's curated five and grows as translations land: it is what the settings screen offers, and a client must not hard-code it. |
