...
| `sessionStarted` | admin-only | `{ "sessionId", "userId", "deviceId"?, "titleId" }` | — (no session list yet) |
| `nowPlaying` | admin-only | `{ "sessionId", "userId", "deviceId"?, "titleId", "positionMs" }` | — |
| `sessionEnded` | admin-only | `{ "sessionId", "userId", "deviceId"?, "titleId" }` | — |
...
`tailnetURL` is always present and nullable, never omitted: `null` is this server *saying* it has no tailnet address.

`id` and `name` are the **Server identity** ([ADR-0034](./adr/0034-server-identity-and-mdns-advertisement.md)). A running server always resolves both at boot, so a client can treat them as present; the wire tag stays `omitempty` only because the internal `Metadata` type itself documents a zero identity as legal for a test harness built outside boot.
...
- **`data`** is the entity's *public* fields only — what the browse API already exposes, plus genres and cast. Deliberately absent: **every path and folder name** (a test greps the whole body for `/`-rooted values), `mtime`, the artwork **bytes** (the image itself is relayed and cached, §3.11), and every enrichment **bookkeeping** column — attempts, retry times, reasons, the id origin, the episode pin, `reviewed`. Those are the sharing Server's notes about how it reached its own answer. Files carry container, codecs, resolution, bitrate, duration and size: what a Capability profile needs, nothing that names disk. A **Show**, **Season**, **Artist**, **Album** and **Movie** (`title`) additionally carry `artworkRoles` (a list ⊆ `poster`/`background`/`logo` for a Show/Season/Artist/Movie, `cover` for an Album) and `artworkVersion` (an opaque cache-bust token) when the sharer has artwork for it — the *signal* the mirror needs to advertise a relayed image, present only when there is one (.scratch/linked-servers issues 19, 20). A Season's roles come from its `entity_artwork` (like a Show's); a Movie's from the `artwork` table (title-keyed). An **Episode** still and a **Track** advertise off other already-copied signals, so neither carries this one.
...
**A linked Show, Season, Artist, Album and Movie advertise their artwork URLs exactly like a local one** — a Show's `posterUrl`/`backgroundUrl`/`logoUrl`, a Season's `posterUrl`, an Artist's `artworkUrl`/`backgroundUrl`/`logoUrl`, an Album's `hasArtwork`, and a Movie's grid `artworkVersion` (the client then requests `/titles/{id}/artwork/poster`) plus its detail `artwork[]` `poster`/`background`/`logo` — for whichever roles the sharer has, and each carries the sharer's `?v={version}` cache-buster ([ADR-0056](./adr/0056-a-linked-library-is-a-read-only-mirror-played-through-a-one-hop-relay.md) §5, .scratch/linked-servers issues 19, 20). GETting one relays the image bytes from the sharer on first request and serves this Server's own cache after, so a client cannot tell a mirrored poster from a local one. A role the sharer lacks is simply not advertised. Every mirrored artwork role — Movie, Show, Season, Artist, Album; Episode still and cast photo — is now advertised and relayed.
...
| --- | --- |
| `GET …/enrichmentCandidates?q=&artist=&page=` — [Admin] | → `200` `{ "candidates": [ { "externalId", "title", "year"?, "thumbnailUrl"? (a same-origin `GET /providerImage` URL, never the provider's host), "disambiguation"?, "kind", "typeLabel"?, "tracklist"?: [ { "disc"?, "position", "title" } ], "releaseId"? } ], "hasMore"? }`. `releaseId` is the exact EDITION a pasted `/release/` URL named ([ADR-0052](./adr/0052-a-chosen-edition-is-part-of-the-record-and-it-licenses-position.md)); it appears only on an album `externalPreview` of such a URL, and is sent straight back as the override's `releaseId`. Page size 12. Blank `q` → empty list. `503 SEARCH_UNAVAILABLE` when the provider is unconfigured/unreachable. **A pasted id or URL is read, not searched:** on the first page the Library's lead is asked to read `q` as a reference (its `external-ref` capability, else the host's own TMDB/MusicBrainz reader when that source leads the kind). If it reads, the answer is `externalPreview`'s — one candidate with `"resolvedRef": true`, which the picker selects, and `externalPreview`'s `400` on a wrong-kind link. Otherwise `q` is searched and `resolvedRef` is absent. An id that reads but names **no record** falls back to searching for `q`, because a bare id is also a search term ("1917" is a film as well as a TMDB id); the `404` comes back only when `q` was pasted as a LINK, or when that search finds nothing either — so a stale bare id still reports itself stale rather than as an empty list. A client never decides what a reference looks like (.scratch/bundled-plugins issue 12). |
| `GET …/externalPreview?ref=` — [Admin] | Resolve a pasted id or URL, read by the Library's lead → `200` single candidate. `400` invalid/kind-mismatch ref, `404` no record, `503`. The web app no longer calls it (the search reads pastes itself); kept for other clients. |
...
| `PUT …/enrichmentOverride` — [Admin] | `{ "externalId", "source", "cascade"?, "releaseId"?, "season"?, "episode"? }` → `200` detail. `source` is the External-id namespace `externalId` belongs to ([ADR-0060](./adr/0060-a-record-id-is-namespaced-and-a-pin-holds-only-a-decision.md) decision 5) — the picked candidate's or paste preview's `source`, echoed back — and is **required**: a namespace no registered Authoritative provider of the item's kind claims is `400`, and a missing `source` is `400`. `releaseId` pins WHICH EDITION of an **Album** decorates its tracks ([ADR-0052](./adr/0052-a-chosen-edition-is-part-of-the-record-and-it-licenses-position.md)) — a release picked from `GET /albums/{id}/editions`, or the one a pasted `/release/` URL names, carried back by `externalPreview` as the candidate's `releaseId` beside the release-group it resolved to. Album only; never identity (the release-group stays what an album IS, [ADR-0038](./adr/0038-album-identity-release-group-wins.md)), so pinning an edition re-keys nothing. **Omitting it CLEARS** a previously chosen edition — a picked search candidate or a pasted `/release-group/` URL is the Admin naming a less specific thing, and a stale edition under a new group would decorate the album from a stranger's tracklist. `season`+`episode` pin WHICH provider episode decorates an **Episode**, overriding the numbers parsed from its filename **for the lookup only** — the fix for a series the provider numbers differently from the files on disk; `source` is not required there — an Episode pick always inherits its Show's namespace instead — but is still validated when sent. identity_key, the Title's own season/episode, its place in the library and every User's watch state are untouched ([ADR-0014](./adr/0014-watch-state-keyed-to-parsed-identity.md)); omit `season`/`episode` to leave any existing pin alone. Durable record pin + immediate re-enrich; never touches identity/watch state; honors Locked fields. Cascade (Show→episodes, Album→tracks, Artist→albums→tracks) is best-effort, surfaced in `cascade` counts. |
...
| `POST /settings/metadata-providers/{slug}/test` — [Admin] | Optional body `{ "apiKey"?, "baseURL"?, "imageBaseURL"? }` (omitted = on-file values). Always `200` `{ "ok", "detail" }` on a completed probe (10s timeout; a down host is `ok:false`, not a 500). No persistence. A provider with a SECOND host is probed against both — the declared probe reference is looked up, then its images are asked for — and a failure on the second names that host, so a dialog with two URL fields has a verdict that can be acted on. |
| `GET /settings/subtitle-providers` — [Admin] | → `200` `{ "providers": [ { "slug", "name", "requiresKey", "enabled", "hasKey", "baseURL", "description", "docsURL" } ], "autoFetchLang" }` (`""` = auto-fetch off). Four further fields describe an **Installed plugin** ([ADR-0058](./adr/0058-an-installed-plugin-is-a-wasm-guest-called-through-a-hand-rolled-abi-on-wazero.md)) and are absent for a Built-in, with exactly the meanings they carry on `/settings/event-sinks`: `installed`, `version`, `disabled` and `lastError`. A download a Plugin answered with more bytes than this server accepts is **refused whole** — nothing is cached and nothing is recorded — and the refusal is what `lastError` says. **`opensubtitles` is a Bundled plugin** since `.scratch/bundled-plugins` issue 09 ([ADR-0059](./adr/0059-the-shipped-metadata-providers-are-bundled-plugins.md) decision 11, as amended), so it now carries those four fields too, and appears on `/settings/plugins` with `origin: "bundled"`; its slug, its settings row, `OBELO_OPENSUBTITLES_API_KEY` / `OBELO_OPENSUBTITLES_BASE_URL` and this endpoint's body are unchanged. It accepts a subtitle of up to 8 MiB, as the Built-in did. A spent OpenSubtitles daily download quota answers `POST /titles/{id}/subtitles/fetch` with `404` ("subtitle no longer available") rather than `500`, and does not count against the plugin. |
...
| `GET /settings/event-sinks` — [Admin] | → `200` `{ "sinks": [ { "slug", "name", "requiresSecret", "enabled", "hasSecret", "url", "events": ["scan.completed"…], "description", "docsURL", "counters": { "delivered", "dropped", "failed" } } ], "availableEvents": ["scan.completed", "enrich.completed", "playback.started", "playback.stopped", "library.changed"] }`. Event sinks ([ADR-0057](./adr/0057-plugins-implement-a-closed-set-of-extension-points-through-a-wire-shaped-contract.md) decision 6). The signing secret is **never returned** — only `hasSecret`. `availableEvents` is the event types **this server can actually derive** — all five curated ones today: it is what the settings screen offers, and a client must not hard-code it, because a future type will exist in the contract one release before its translation does. `counters` is the sink's delivery tally **since boot** (in-memory, reset by a restart): `delivered` climbing is a working receiver, `failed` a target that refuses or cannot be reached, `dropped` a target too slow for the bounded queue. It takes the place of the `/test` leaf a sink cannot have. Four further fields describe an **Installed plugin** ([ADR-0058](./adr/0058-an-installed-plugin-is-a-wasm-guest-called-through-a-hand-rolled-abi-on-wazero.md)) and are absent for a Built-in: `installed` (this sink is a module under `<dataDir>/plugins/<slug>/`, not code compiled into the server), `version` (the author's own version string), `disabled` (this server will not call it — it was refused at load, or it failed enough times at runtime to be stopped) and `lastError` (the sentence that says why). `disabled` is **not** the Admin's `enabled` toggle: a Plugin can be switched on and still be disabled, which is exactly the state that needs explaining. |
| `PUT /settings/event-sinks` — [Admin] | Partial update; per sink `{ "slug", "enabled"?, "secret"?, "url"?, "events"? }` — pointer semantics: omitted = unchanged, `""` = clear, value = set; `events` omitted = unchanged, `[]` = subscribe to nothing. All-or-nothing validation before any write. → `200` (GET shape). Errors: `422 PROVIDER_UNKNOWN` / `PROVIDER_KEY_REQUIRED` (enabling a sink with no signing secret) / `PROVIDER_INVALID_BASE_URL` (a target that is not an absolute http(s) URL) / `PROVIDER_INVALID_SETTING` (enabling with no target, or subscribing to an event this server does not emit). **Hot-reloads** the live sinks — no restart. There is deliberately **no** `/test` leaf: a provider probe asks a third party whether a credential works, while a sink's secret is this server's own signing key. |
| `GET /settings/plugins` — [Admin] | → `200` `{ "plugins": [ { "id", "name", "version"?, "apiVersion"?, "provides": ["event-sink"…], "enabled", "disabledByFailure", "lastError"?, "source"?, "installedAt"?, "origin"?, "state"? } ] }`. **Installed plugins** ([ADR-0058](./adr/0058-an-installed-plugin-is-a-wasm-guest-called-through-a-hand-rolled-abi-on-wazero.md), `.scratch/plugin-system` issue 10) — the WebAssembly modules an Admin put on this server, as opposed to the Built-ins compiled into it. This surface is **only** how code gets onto the server: what a plugin *does* is configured on the screen for its Extension point (`/settings/event-sinks`, `/settings/metadata-providers`, `/settings/subtitle-providers`), through the same endpoint and the same body as the Built-in beside it. `enabled` is the **Admin's switch**, durable across a restart; `disabledByFailure` is **this server refusing to call it** — refused at load, or stopped after repeated failures — and `lastError` says why. The two are not the same and a client must not merge them: a plugin can be enabled and stopped at once. **`lastError` may also be present on a plugin that is running perfectly well**, and a client must not read it as "stopped": since 2026-09-18 a Metadata provider guest that runs to completion and cleanly answers an error — a rejected key, a document it cannot parse — parks that one item and puts its sentence here without costing the plugin a strike ([ADR-0058](./adr/0058-an-installed-plugin-is-a-wasm-guest-called-through-a-hand-rolled-abi-on-wazero.md) decision 7, as amended). It is retired by the next call that works. `disabledByFailure` is the only field that says this server has stopped calling a plugin. `provides` is the Extension points the manifest declared, in the contract's own tokens. `source` is `upload`, the manifest URL that was pasted, or `placed by hand`. **`origin` is who put the plugin here** ([ADR-0059](./adr/0059-the-shipped-metadata-providers-are-bundled-plugins.md), `.scratch/bundled-plugins`): `bundled` for one this server shipped and installed itself on first boot, `admin` for one a person uploaded, pasted a URL for or placed by hand. It is what a screen turns into *"Shipped with Obelo"* where an Admin's plugin shows the file or URL it came from, and it is the ONLY difference an operator sees: a Bundled plugin is an Installed plugin in every other respect — same sandbox, same allowlist, same contract, same lifecycle, same row, same settings. A client must not read `bundled` as read-only; it can be disabled, configured and uninstalled like any other. **`state` is the row's lifecycle state when it is not simply installed**, and today it has exactly one value: `declined` — a Bundled plugin the Admin UNINSTALLED. That row is not an installed plugin: it carries `origin: "bundled"`, its id and name, and nothing else is meaningful on it (no `enabled`, no `version`, no settings), because there are no files on disk. It exists so the screen can offer the one verb below instead of the plugin silently vanishing, and so the next boot knows not to put it back. It sorts among the installed rows by id — a declined plugin is not a second kind of thing. `state` is absent for every installed plugin. A plugin whose manifest declares settings **of its own** (`.scratch/plugin-system` issue 13) also carries `settingsSchema: [ { "key", "type": "string"|"secret"|"url"|"bool"|"enum"|"multi-select"|"integer", "label"?, "help"?, "required"?, "default"?, "options"?, "min"?, "max"? } ]` — the ordered field list, straight from the file on disk — and `settings: { "values": { … }, "secrets": { "<key>": true|false } }`. `values` are **effective**: what is saved, falling back to the manifest's declared default, because that is what the guest will read. A **secret field's value is never in any response** — `secrets` says only whether one is on file, exactly as `hasApiKey` does for a metadata provider. A plugin that declares no fields carries neither key. A plugin whose signature **verified against a pinned publisher key** (`.scratch/plugin-system` issue 15) also carries `publisher` and `keyId`. Both are **absent unless a signature was actually checked** — a plugin installed while no keys were pinned has neither even if a signature travelled with it — so a client must read their absence as "nobody verified this", **never** as "unsigned", which is a different claim and not one this server is in a position to make. |
| `POST /settings/plugins` — [Admin] | **multipart/form-data** with two file parts: `manifest` (the `manifest.json`, byte for byte as the author wrote it) and `module` (the `.wasm`), plus an **optional** third, `signature` (the detached `plugin.sig.json`, ≤ 8 KiB). Named parts rather than an archive, so the server never unpacks a path it did not choose. Omitting `signature` is not an error and is the usual case: whether this server requires one is decided by `GET/PUT /settings/plugins/publishers`. → `201` (GET shape, the whole list). The module is compiled in a staging directory **before** anything is installed, so a refusal leaves nothing on disk and nothing in the database. Errors: `422 PLUGIN_INVALID_MANIFEST` / `422 PLUGIN_API_VERSION` (the message **names which side to upgrade**) / `409 PLUGIN_DUPLICATE` (the id is already claimed, by an Installed plugin or a Built-in) / `422 PLUGIN_INVALID_MODULE` (no module, or one that will not compile or instantiate) / `422 PLUGIN_SIGNATURE` (publisher keys are pinned and this plugin does not satisfy them) / `400 BAD_REQUEST` (a missing part, named) / `413` (a part over the cap: 64 MiB for the module, 256 KiB for the manifest). **Takes effect with no restart** — the registry is rebuilt and swapped, and the provider, subtitle and sink Managers reload. |
| `POST /settings/plugins/from-url` — [Admin] | `{ "url", "signatureUrl"? }` — the URL of a plugin's `manifest.json`; its module is fetched from **beside it** (the manifest's `module` name, default `plugin.wasm`, resolved against the manifest's directory), and its detached signature likewise from `plugin.sig.json` in the same directory, where **a 404 simply means unsigned** and is not an error. `signatureUrl` overrides only that third address, for a catalog entry that carries one, and is address-checked exactly as the manifest URL is. → `201` (GET shape). Every fetch goes through `internal/safefetch`, so the bounded redirect chain and the private-address refusal on every hop apply. **Unlike every other outbound fetch in this server, the first hop is address-checked too**: a URL resolving into loopback / RFC1918 / link-local space is refused, because what comes back is code this server executes. Upload the file instead. **This is also the whole of a catalog install** — an entry is a manifest URL, so browsing a catalog adds a way to choose an address and adds nothing to this path. Errors: `422 PLUGIN_SOURCE_REFUSED` (not an absolute http(s) URL, unresolvable, a private address, a non-200 on the manifest or module, or a response over the cap), plus every error the upload can give. |
| `POST /settings/plugins/{id}/enable` — [Admin] | The Admin's switch, on. → `200` (GET shape). `404 PLUGIN_UNKNOWN` for a plugin that is not installed. |
| `POST /settings/plugins/{id}/disable` — [Admin] | The Admin's switch, off. The plugin is **not registered at all** in the rebuilt registry, so delivery stops immediately — it is not a flag the delivery path consults — and it disappears from its Extension point's settings screen while staying on this one, named, so it can be switched back on. → `200` (GET shape). |
| `POST /settings/plugins/{id}/reenable` — [Admin] | Forgive a recorded failure: clears `disabledByFailure`, `lastError`, the consecutive-failure count and the allowlist-violation count, **and reloads the plugin from disk**, so a plugin whose manifest or module was fixed gets a genuine second try. It does **not** touch the Admin's `enabled` switch. → `200` (GET shape). |
| `POST /settings/plugins/{id}/reinstall-shipped` — [Admin] | Put back a **Bundled** plugin this server ships and the Admin uninstalled ([ADR-0059](./adr/0059-the-shipped-metadata-providers-are-bundled-plugins.md) decision 2). It clears the declined mark, writes the shipped module and manifest into the data directory exactly as first boot would, inserts the row with `origin: "bundled"`, and **rebuilds and swaps the registry**, so the provider is composed into its chain again with no restart. → `200` (GET shape). It is the ONLY way back, which is why it is a route: uninstalling a Bundled plugin writes a declined mark precisely so that neither the next boot nor a server upgrade undoes the Admin's decision, and without a verb to clear it an operator who changed their mind would have to edit the database. Errors: `404 PLUGIN_UNKNOWN` (this server ships no plugin with that id — including for an id that exists but was never bundled), `409 PLUGIN_DUPLICATE` (something is already installed under that id; uninstall it first, which is also what an Admin's own same-id upload gets). Settings, per-Library overrides and item pins that survived the uninstall are keyed by the same id and are still there.
| `PUT /settings/plugins/{id}/settings` — [Admin] | `{ "values": { "<key>": <value> } }` — the values of the settings the plugin's **own manifest declared**, each in the JSON shape its type names: a JSON string for `string`/`secret`/`url`/`enum`, `true`/`false` for `bool`, a JSON **number** with no fractional part for `integer` (`"7"` is refused, not coerced), an array of strings for `multi-select`. This is **not** the Extension point's settings — those stay the fixed shape, on the screen for the seam. A key the body **omits** is left as it is, which is the only way a secret the server never returned survives a save; an explicit `null` clears it. Validated server-side against the manifest — required, enum membership (and every element of a multi-select), integer range, the shape of a URL — and **nothing is written unless every field passes**. → `200` (GET shape, the whole list). Errors: `400 PLUGIN_INVALID_SETTINGS`, whose `details` carry `{ "fields": [ { "key", "message" } ] }` and whose `message` restates the first of them, so a form can put each sentence under the control that caused it; `404 PLUGIN_UNKNOWN`. Takes effect on the plugin's **next call**, with no rebuild and no restart. |
| `DELETE /settings/plugins/{id}` — [Admin] | Uninstall: the module is unloaded, the directory is deleted, and the `plugins`, `plugin_settings` **and the Extension point's own settings row** go with it (an `event_sinks` row left behind would strand a signing secret under a slug no plugin claims). Artwork and subtitles the plugin produced **stay** in the ordinary caches — they are identity-keyed and they are the library's now. **Uninstalling a plugin whose `origin` is `bundled` also records that the Admin declined it** ([ADR-0059](./adr/0059-the-shipped-metadata-providers-are-bundled-plugins.md)), so neither the next boot nor a server upgrade puts it back; it then appears in the list as a `state: "declined"` row and `POST /settings/plugins/{id}/reinstall-shipped` is what undoes it. Installing anything under that id — the reinstall verb, or the Admin's own upload — clears the mark. → `200` (GET shape). `404 PLUGIN_UNKNOWN`. |
| `GET /settings/plugins/catalog` — [Admin] | → `200` `{ "url", "entries": [ { "id", "name", "version"?, "publisher"?, "provides": ["event-sink"…]?, "manifestUrl", "signatureUrl"?, "description"?, "docsUrl"? } ], "error" }`. The **optional catalog** (`.scratch/plugin-system` issue 15): a JSON index an operator pointed this server at. **Obelo publishes none and recommends none** ([ADR-0001](./adr/0001-fully-self-hosted-no-vendor-dependency.md)) — `url` is `""` until an Admin sets one, and while it is empty no request is made and no Browse tab is shown. **A CATALOG THAT CANNOT BE READ IS A `200` WITH A NOTE**, never a 5xx: `entries: []` and a sentence in `error`, because the upload and paste-URL paths have nothing to do with the catalog and must not be taken away by somebody else's outage. A client renders `error` as a note, not as a failed request, and keys the Browse tab off `url` rather than off `entries` — a configured catalog that is unreachable still has a tab, and the tab is where the note belongs. The index itself is **data** and is fetched under the ordinary `internal/safefetch` policy (first hop not address-checked, so an operator's own index on their own LAN works); installing an entry is not, see below. Capped at 1 MiB with a 10 s timeout. Entries are returned in the index's own order; one with no `manifestUrl` is dropped, because nothing could be done with it. |
| `PUT /settings/plugins/catalog` — [Admin] | `{ "url" }` — set the catalog, or clear it with `""`. The address is validated as an absolute http(s) URL but **NOT fetched**, so saving the address of a catalog that happens to be down is not an error. → `200` (GET shape, with the index freshly fetched, note and all). `422 PLUGIN_SOURCE_REFUSED` for something that is not an absolute http(s) URL. **There is no "install this entry" route, deliberately**: an entry is a manifest URL, so installing one is `POST /settings/plugins/from-url` with that URL — the same endpoint, the same safe-fetch policy, the same **first-hop address check**, the same refusals. An entry resolving into this server's own network is refused with the sentence a pasted address gets. |
| `GET /settings/plugins/publishers` — [Admin] | → `200` `{ "publishers": [ { "publisher", "publicKey", "keyId"?, "addedAt"? } ] }`. The **pinned publisher keys** (`.scratch/plugin-system` issue 15). **AN EMPTY LIST IS THE DEFAULT POLICY, not missing data**: with nothing pinned, no install is signature-checked at all and the server behaves exactly as it did before signing existed. `publicKey` is a base64 ed25519 key and is returned **in full, unmasked** — the one credential-shaped field in this API that is, because it is public and an operator has to compare it against what a publisher advertises. `keyId` is a short fingerprint (the first eight bytes of the key's SHA-256, in hex) and **decides nothing**; verification uses the key's own bytes. |
| `PUT /settings/plugins/publishers` — [Admin] | `{ "publisher", "publicKey" }` — pin a key, **replacing** whatever that publisher had, which is what a key rotation needs (the old key stops being accepted at once). Matching on `publisher` is case-insensitive. → `200` (GET shape). `422 PLUGIN_SIGNATURE` for an empty name or something that is not an ed25519 public key. **Pinning the first key changes the policy for every later install**: from then on an install must carry a detached `plugin.sig.json` naming a pinned publisher and verifying under that publisher's key, over the exact manifest and module bytes — the ed25519 signature covers `"obelo-plugin-v1\n" ‖ sha256(manifest) ‖ sha256(module)`, and the manifest is stored byte for byte as its author shipped it, which is why the signature is **detached** and could not be a field of the document it covers. A failure is `422 PLUGIN_SIGNATURE` **naming the publisher the plugin claimed**, in one of four sentences: no signature at all, a publisher nobody pinned, a signature covering different files, or one that does not verify. An **already-installed** plugin is **never re-verified** — not at boot, not on enable, disable or re-enable — because a change of mind about future installs must not become an outage of present ones. A verified install records `publisher`/`keyId` on `GET /settings/plugins`; an unverified signature records nothing, though the file is still kept beside the manifest as provenance. See **[docs/plugins/authoring.md](./plugins/authoring.md)** §7a. |
| `DELETE /settings/plugins/publishers/{publisher}` — [Admin] | Unpin a key. → `200` (GET shape). `404 PLUGIN_UNKNOWN` for a publisher nobody pinned. Removing the **last** key returns the server to its default policy: nothing is verified. |
